...
@external
func add_edge{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
        _src : felt, _dst : felt) -> (edge_id : felt):
...
@external
func are_adjacent{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
        _src : felt, _dst : felt, _last_edge_id : felt) -> (connected : felt):
    alloc_locals
    if _last_edge_id == 0:
        return (0)
    end
...
@external
func reachable{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
        _src : felt, _dst : felt, _discovered_len : felt, _discovered : felt*) -> (
        connected : felt, new_discovered_len : felt):
    alloc_locals

    assert _discovered[_discovered_len] = _src  # assignement
    ...
    let (b, new_discovered_len) = inner_function(
        _src, _dst, _discovered_len + 1, _discovered, edge_n)

    return (b, new_discovered_len)
end

func inner_function{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
        _src : felt, _dst : felt, _discovered_len : felt, _discovered : felt*, edge_id : felt) -> (
        connected : felt, new_discovered_len : felt):
    alloc_locals
    ...
    let (b, new_discovered_len) = inner_function(
        _src, _dst, _discovered_len, _discovered, edge_id - 1)

    return (b, new_discovered_len)
end

func contains{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
        _arr_len : felt, _arr : felt*, _val : felt) -> (b : felt):
    if _arr_len == 0:
        return (0)
    end
    if _arr[_arr_len - 1] == _val:
        return (1)
    end
    return contains(_arr_len - 1, _arr, _val)
end
